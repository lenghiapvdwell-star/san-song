...
st.set_page_config(page_title="V32.8 - Giao diện FireAnt", layout="wide")
...
def calculate_full_signals(df, vni_df):
    ...
    df['is_buy'] = (c > df['ma20']) & (df['ma20'] > df['ma50'] * 0.99) & (v > v.rolling(20).mean() * 1.3) & (df['rsi'] > 45)
    
    return df

# --- SIDEBAR ---
with st.sidebar:
    st.header("⚡ V32.8 PRO CHART")
    if st.button("🔄 UPDATE & GHI ĐÈ GITHUB"):
        st.info("Đang xử lý...") # Giữ logic GitHub của bạn ở đây
...
try:
    ...
    if mode == "🌟 SIÊU SAO THEO DÕI":
        # (Giữ nguyên logic lọc siêu sao của bản 32.5)
        st.subheader("🚀 BỘ LỌC SIÊU SAO V32")
        # ... hiển thị bảng ...
        ...
        if df_c is not None:
            ...
            fig.add_trace(go.Scatter(x=bombs['date'], y=bombs['high']*1.02, mode='text', text="💣", textfont=dict(size=22), name="Bomb"), row=1, col=1)
            ...
            # --- CẤU HÌNH GIAO DIỆN KÉO DÃN KIỂU FIREANT/TRADINGVIEW ---
            fig.update_layout(
                height=850,
                template="plotly_dark",
                xaxis_rangeslider_visible=False,
                dragmode='pan', # Mặc định là nắm kéo
                hovermode='x unified',
                # Mở khóa các trục để kéo dãn tự do
                xaxis=dict(fixedrange=False, autorange=True), 
                yaxis=dict(fixedrange=False, autorange=True, side='right'), # Giá nằm bên phải như Fireant
                yaxis2=dict(fixedrange=False, autorange=True, side='right'),
                yaxis3=dict(fixedrange=False, autorange=True, side='right'),
                yaxis4=dict(fixedrange=False, autorange=True, side='right'),
                margin=dict(l=10, r=50, t=30, b=10)
            )

            # Cấu hình Toolbar cho phép chuyển đổi Zoom/Pan
            config = {
                'scrollZoom': True,         # Lăn chuột để phóng to/thu nhỏ
