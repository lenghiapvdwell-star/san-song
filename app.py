...
st.set_page_config(page_title="Hệ Thống Săn Sóng V32.1", layout="wide")

# --- HÀM TÍNH TOÁN (GIỮ NGUYÊN V32) ---
...
# --- HIỂN THỊ CHÍNH (SỬA LỖI TẠI ĐÂY) ---
try:
    ...
    # Chỉ chạy quét rũ hàng KHI nhấn nút VÀ ĐÃ có dữ liệu hose_df
    if check_ru:
        ...
        for s in hose_df['symbol'].unique():
            ...
            if df_s is not None:
                l = df_s.iloc[-1]
                # Logic rũ hàng chuẩn terminal: RSI thấp + Vol kiệt + Giá giảm sâu
                cond_price = (l['close'] < df_s['close'].shift(1))
                cond_vol = (l['volume'] < df_s['volume'].rolling(20).mean() * 0.85)
                cond_rsi = (l['rsi'] < 40)
                
                if (cond_price and cond_vol) or cond_rsi:
                    list_ru.append({
                        "Mã": s, "Giá": int(l['close']), "RS": l['rs'], "RSI": round(l['rsi'],1),
                        "Tình trạng": "Rũ hàng/Kiệt Vol"
                    })
        if list_ru:
            st.table(pd.DataFrame(list_ru).sort_values("RSI"))
        else:
            st.info("Chưa tìm thấy mã rũ hàng.")
